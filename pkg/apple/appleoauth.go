...
// VerifyJWT ...
func VerifyJWT(token, email string) (res string, err error) {
	...
	if exp < time.Now().Unix() {
		return res, errors.New("Apple token already expired")
	}
	emailVerified := claims["email_verified"].(string)
	if emailVerified != "true" {
		return res, errors.New("Email is not verified")
	}
	res = claims["email"].(string)
