...
func (repository CouponRedeemRepository) SelectReport(c context.Context, parameter models.CouponRedeemParameter) (data []models.CouponRedeemReport, err error) {
	...
	if parameter.StartDate != "" && parameter.EndDate != "" {
		conditionString += ` AND (DEF.REDEEMED_AT::DATE BETWEEN '` + parameter.StartDate + `' AND '` + parameter.EndDate + `' OR DEF.REDEEMED_AT IS NULL)`
	} else {
		conditionString += ` AND (DEF.REDEEMED_AT::DATE BETWEEN date_trunc('MONTH',now())::DATE AND now()::date OR DEF.REDEEMED_AT IS NULL)`
	}
