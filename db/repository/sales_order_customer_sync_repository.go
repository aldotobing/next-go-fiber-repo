...
type ISalesOrderCustomerSyncRepository interface {
	...
	InsertWithLine(c context.Context, model *models.SalesOrderCustomerSync) (res string, err error)
	ReInsertWithLine(c context.Context, model *models.SalesOrderCustomerSync) (res string, err error)
	MergeData(c context.Context, model *models.SalesOrderCustomerSync) (res string, modifyOnly int, err error)
...
func (repository SalesOrderCustomerSyncRepository) InsertWithLine(c context.Context, model *models.SalesOrderCustomerSync) (res string, err error) {
...
func (repository SalesOrderCustomerSyncRepository) ReInsertWithLine(c context.Context, model *models.SalesOrderCustomerSync) (res string, err error) {
	...
	if model.ID != nil && *model.ID != "" {
		...
		deletedRow, _ := transaction.QueryContext(c, deletelinestatement, model.ID)
		deletedRow.Close()

		deleteheaderstatement := `delete from sales_order_header WHERE id = $1`

		deletedHeaderRow, _ := transaction.QueryContext(c, deleteheaderstatement, model.ID)
		deletedHeaderRow.Close()
...
func (repository SalesOrderCustomerSyncRepository) MergeData(c context.Context, model *models.SalesOrderCustomerSync) (res string, modifyOnly int, err error) {
	...
	if availableso.ID != nil {
		model.ID = availableso.ID
		if *model.OperationType == "2" {
			var cdata string
			fmt.Println("operation 2 update")

			updatestmt := ` update sales_order_header set status=$1
			
			WHERE id = $2 returning id `

			err = repository.DB.QueryRowContext(c, updatestmt, str.EmptyString(*model.Status),
				availableso.ID).Scan(&cdata)
			modifyOnly = 1
		} else {
			res, err = repository.ReInsertWithLine(c, model)
			modifyOnly = 0
		}
	} else {
