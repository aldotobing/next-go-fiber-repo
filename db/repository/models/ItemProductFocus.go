...
AND IP.ITEM_ID = IUL.ITEM_ID`
	...
	SELECT I.ID,
		...
		array_to_string((array_agg(U.ID || '#sep#' || u."_name" || '#sep#' || IUL.conversion::text || '#sep#' || ip.price::text || '#sep#' || ip.price_list_version_id || '#sep#' || IUL.visibility || '#sep#' || ip.modified_date order by iul."conversion" asc)),'|') AS additional_data,
        td.MULTIPLY_DATA
	FROM PRODUCT_FOCUS DEF
	JOIN ITEM I ON I.ID = DEF.ITEM_ID
	JOIN ITEM_CATEGORY IC ON IC.ID = I.ITEM_CATEGORY_ID
