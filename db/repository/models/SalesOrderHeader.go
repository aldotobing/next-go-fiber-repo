...
// SalesOrderHeaderParameter ...
type SalesOrderHeaderParameter struct {
	ID         string `json:"id_customer_order_header"`
	DocumentNo string `json:"document_no"`
...
var (
	// SalesOrderHeaderOrderBy ...
	SalesOrderHeaderOrderBy = []string{"def.id", "def.created_date"}
	// SalesOrderHeaderOrderByrByString ...
	SalesOrderHeaderOrderByrByString = []string{
		"def.id",
	}

	SalesOrderHeaderSelectStatement = ` select 
	...
	'' as void_reason, 2 as order_source
	...
	// SalesOrderHeaderWhereStatement ...
	SalesOrderHeaderWhereStatement = `WHERE def.created_date IS not NULL`
)
