...
// CustomerTypeRoutes ...
type CustomerTypeRoutes struct {
	RouterGroup fiber.Router
	Handler     handlers.Handler
}

// RegisterRoute register CustomerType routes
func (route CustomerTypeRoutes) RegisterRoute() {
	handler := handlers.CustomerTypeHandler{Handler: route.Handler}
	// jwtMiddleware := middlewares.JwtMiddleware{ContractUC: handler.ContractUC}

	r := route.RouterGroup.Group("/api/apps/customertype")
	...
	r2 := route.RouterGroup.Group("/api/web/customertype")
	// r.Use(jwtMiddleware.VerifyUser)
